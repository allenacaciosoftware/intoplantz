...
class SearchHandler
{
    ...
    {
        $postsInQueryPart = '';
        array_walk( $taxonomies, [ $this, 'addToQueryFormatted' ] );
        $taxonomies = implode( ' OR ', $taxonomies );
        $taxonomyIds = $this->wpdb->get_col( $this->wpdb->prepare( "SELECT term_taxonomy_id AS id FROM {$this->wpdb->term_taxonomy}\n              INNER JOIN {$this->wpdb->terms} ON (wp_term_taxonomy.term_taxonomy_id = wp_terms.term_id AND ({$taxonomies}))\n              WHERE wp_terms.name LIKE %s", '%' . $this->searchWord . '%' ) );
        
        if ( !empty($taxonomyIds) ) {
            $termIds = implode( ',', $taxonomyIds );
            $productsIds = $this->wpdb->get_col( "SELECT ID FROM {$this->wpdb->posts}\n           INNER JOIN {$this->wpdb->term_relationships} ON (wp_posts.ID = wp_term_relationships.object_id)\n           WHERE wp_term_relationships.term_taxonomy_id IN ({$termIds}) AND post_type = 'product'" );
        }
    ...
    {
        $item = "wp_term_taxonomy.taxonomy = '{$item}'";
    }
